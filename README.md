...
Lab2B写的还行，整整写了24小时，除了中间睡觉睡了6小时之外， 连吃饭的时候都在想问题。甚至睡觉的时候都他娘的和别人在讨论这个问题。前面在并发写入的地方出现了顺序问题。 早晨起来灵光乍现，想到用生产者消费者模式来解决并发写入时可能带来的顺序错误问题。 最终结果还不错。 只不过唯一比较坑的是， 由于本次代码里面的协议是我自己设计的，没有用MIT课程给的协议，导致RPC消息byte总量总是过大，但是我也不打算去重新修改整个通信协议了， 没什么太大意义。 其他的都是能够正常过的。 
